...
import { HelperBlock } from './models/helper-block';
...
export class ResizableDirective implements OnInit, OnChanges, OnDestroy, AfterViewInit{
  ...
  private _handleResizing?: ResizeHandle;
  private _direction?: { 'n': boolean, 's': boolean, 'w': boolean, 'e': boolean };
  private _directionChanged?: { 'n': boolean, 's': boolean, 'w': boolean, 'e': boolean };
  private _aspectRatio = 0;
  private _containment?: HTMLElement;
  private _origMousePos?: Position;

  /* Original Size and Position */
  private _origSize?: Size;
  private _origPos?: Position;

  /* Current Size and Position */
  private _currSize?: Size;
  private _currPos?: Position;

  /* Initial Size and Position */
  private _initSize?: Size;
  private _initPos?: Position;

  /* Snap to gird */
  private _gridSize?: IPosition;
   ...
   */
  @Input() rzContainment?: string | HTMLElement;

  /*
   * Snaps the resizing element to a grid, every x and y pixels.
   * A number for both width and height or an array values like [ x, y ]
   */
  @Input() rzGrid?: number | number[];
  ...
  private unsubscribeEvents() {
    ...
    this.draggingSub = undefined;
  }

  onMouseLeave() {
    if (this._handleResizing) {
      this.stopResize();
      this._origMousePos = undefined;
  ...
  private stopResize() {
    ...
    this._handleResizing = undefined;
    this._direction = undefined;
    this._origSize = undefined;
    this._origPos = undefined;
    if (this._containment) {
      this.resetBounding();
    }
  }
  ...
  private checkBounds() {
    if (this._containment) {
      ...
      if (this._direction && this._direction.n && (this._currPos!.y + this._bounding.translateY < 0)) {
